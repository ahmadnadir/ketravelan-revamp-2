...
export async function fetchUserTrips(userId: string) {
  ...
  const { data: createdTrips, error: createdError } = await supabase
    .from('trips')
    .select('id, title, destination, cover_image, start_date, end_date, price, currency, max_participants, current_participants, tags, travel_styles, requirements, status, visibility, type, slug, created_at')
  ...
  const { data: memberTrips, error: memberError } = await supabase
    ...
    .select(`
      ...
      trip:trips(
        ...
        tags, travel_styles, requirements, status, visibility, type, slug, created_at
      )
    `)
    .eq('user_id', userId)
    .is('left_at', null);
