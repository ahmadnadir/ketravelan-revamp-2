...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as ExpenseAddedRequest;
    if (!body?.expenseId) {
    ...
    const userIds = Array.from(new Set([payerId, ...participantIds]));
    ...
    const totalOwed = (participants || [])
      ...
      .reduce((sum: number, p: any) => sum + Number(p.amount_owed || 0), 0);

    const results: Array<{ userId: string; email?: string; status: string; role: string }> = [];
    ...
    const sendToUser = async (userId: string, subject: string, messageHtml: string, preheader: string) => {
      ...
      results.push({ userId, email, status: body.dryRun ? "preview" : "sent", role: "recipient" });
    };
    ...
    for (const participant of participants || []) {
      ...
      await sendSystemPush({
        ...
        body: `${expense.description} - You owe ${expense.currency} ${amountOwed}`,
        actionUrl: `/trip/${trip.slug || trip.id}/hub?tab=expenses`,
        priority: "high",
        metadata: {
          trip_id: trip.id,
