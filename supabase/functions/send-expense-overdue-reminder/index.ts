...
interface ExpenseOverdueRequest {
  expenseId: string;
  daysOverdue?: number;
  memberId?: string;
  remindAll?: boolean;
...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as ExpenseOverdueRequest;
    if (!body?.expenseId) {
    ...
    const unpaid = (participants || []).filter((p: any) => !p.is_paid);
    const targetIds = body.memberId
      ? unpaid.filter((p: any) => p.user_id === body.memberId)
      : unpaid;

    const recipientIds = Array.from(new Set(targetIds.map((p: any) => p.user_id)));
    ...
    for (const participant of targetIds) {
      ...
      await sendSystemPush({
        ...
        title: "Expense Overdue Reminder",
        body: `Payment reminder ⏰ You still owe ${expense.currency}${amountOwed} for ${expense.description}`,
