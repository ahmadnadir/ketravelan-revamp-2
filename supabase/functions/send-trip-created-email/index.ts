...
function buildHtmlEmail(opts: {
  ...
  const preheader = `Your trip on ${brand} is live!`;
  ...
  return [
    ...
    `<a href="${ctaUrlEsc}" target="_blank" style="display:inline-block;padding:14px 26px;border-radius:10px;background:#000000;color:#ffffff;text-decoration:none;font-weight:600">View Your Trip</a>`,
    '</td></tr></table>',
    '</td></tr>',
    '<tr><td style="padding:24px 28px;font-size:12px;color:#64748b;line-height:1.6">',
    'Your trip is now live and ready for people to join!<br><br>',
...
interface TripCreatedEmailRequest {
  tripId: string;
  dryRun?: boolean;
}
...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as TripCreatedEmailRequest;
    if (!body?.tripId) {
    ...
    const { data: trip, error: tripErr } = await admin
      .from("trips")
      .select("id, title, destination, creator_id, visibility, slug")
      .eq("id", body.tripId)
      .maybeSingle();
    ...
    // Fetch creator profile for name and email preferences
    ...
    const subject = `Your trip "${trip.title}" is now live! 🚀`;
    
    const greet = `Hi <strong>${escapeHtml(creatorName)}</strong>,<br><br>`;
    const tripInfo = `Your trip to <strong>${escapeHtml(trip.destination || "Adventure Land")}</strong> is now published and ready for travelers to join!<br><br>Share it with friends or let travelers discover it on Ketravelan.`;
    
    const html = buildHtmlEmail({
      brand: "Ketravelan",
      title: "Your Trip is Live! 🎉",
      messageHtml: `${greet}${tripInfo}`,
      ctaUrl: tripUrl,
    ...
    const text = [
      ...
      `Hi ${creatorName},`,
      "",
      `Your trip to ${trip.destination || "Adventure Land"} is now published and ready for travelers to join!`,
      "",
      "Share it with friends or let travelers discover it on Ketravelan.",
      "",
      `View trip: ${tripUrl}`,
      "",
      "The Ketravelan Crew",
    ...
    if (body.dryRun) {
      return new Response(
        JSON.stringify({ ok: true, preview: { html, text, creatorEmail, tripUrl } }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }
