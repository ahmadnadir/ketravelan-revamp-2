...
function buildHtmlEmail(opts: {
  ...
  const preheader = `Trip complete on ${brand}`;
...
interface TripEndedExpensesCompleteRequest {
  tripId: string;
  dryRun?: boolean;
}
...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as TripEndedExpensesCompleteRequest;
    if (!body?.tripId) {
    ...
    const { data: trip, error: tripErr } = await admin
      .from("trips")
      .select("id,title,destination,creator_id,slug,cover_image,end_date")
      .eq("id", body.tripId)
      .maybeSingle();
    ...
    if (creatorProfile?.trip_reminders === false || creatorProfile?.email_notifications === false) {
    ...
    const subject = `Trip complete: ${trip.title}`;
    const messageHtml = `Hi <strong>${escapeHtml(creatorName)}</strong>,<br><br>Your trip to <strong>${escapeHtml(trip.destination || "Adventure Land")}</strong> wrapped on <strong>${escapeHtml(endDateStr)}</strong>, and all expenses are settled.<br><br>Nice work keeping everything tidy. You can review the trip summary anytime.`;

    const html = buildHtmlEmail({
      brand: "Ketravelan",
      title: "Trip Complete + Expenses Settled",
      messageHtml,
      ctaUrl: tripUrl,
    ...
    const text = [
      "Trip complete + expenses settled",
      "",
      `Hi ${creatorName},`,
      `Your trip to ${trip.destination || "Adventure Land"} wrapped on ${endDateStr}, and all expenses are settled.`,
      "Review the trip summary anytime.",
      "",
      `View trip: ${tripUrl}`,
      "",
      "The Ketravelan Crew",
