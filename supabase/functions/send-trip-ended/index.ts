...
function buildHtmlEmail(opts: {
  ...
  const preheader = `Trip wrapped up on ${brand}`;
...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as TripEndedRequest;
    if (!body?.tripId) {
    ...
    const messageHtml = `Hi <strong>${escapeHtml(creatorName)}</strong>,<br><br>Your trip to <strong>${escapeHtml(trip.destination || "Adventure Land")}</strong> wrapped on <strong>${escapeHtml(endDateStr)}</strong>.<br><br>Hope it was a blast. You can review the trip details and keep any final notes or expenses in one place.`;
    ...
    const text = [
      ...
      "",
      `Hi ${creatorName},`,
      `Your trip to ${trip.destination || "Adventure Land"} wrapped on ${endDateStr}.`,
      "Review the trip details and keep any final notes or expenses in one place.",
      "",
      `View trip: ${tripUrl}`,
      "",
      "The Ketravelan Crew",
