...
function buildHtmlEmail(opts: {
  ...
  const preheader = `Invite accepted on ${brand}`;
...
serve(async (req: Request) => {
  ...
  try {
    const body = await req.json() as InviteAcceptedRequest;
    if (!body?.tripId || !body?.inviteeId) {
      return new Response(JSON.stringify({ error: "Missing tripId or inviteeId" }), {
        status: 400,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }
    ...
    const { data: inviteeProfile, error: inviteeErr } = await admin
      .from("profiles")
      .select("full_name, username")
      .eq("id", body.inviteeId)
      .maybeSingle();
    if (inviteeErr) throw inviteeErr;

    const inviteeName = inviteeProfile?.full_name || inviteeProfile?.username || "A traveler";
    const tripIdentifier = trip.slug || trip.id;
    const tripUrl = `${SITE_ORIGIN}/trip/${tripIdentifier}`;

    const subject = `Invite accepted: ${trip.title}`;
    const messageHtml = `<strong>${escapeHtml(inviteeName)}</strong> accepted your invitation for <strong>${escapeHtml(trip.title)}</strong> (${escapeHtml(trip.destination || "")}).`; 
    ...
    const text = [
      `Invite accepted for ${trip.title}`,
      `Invitee: ${inviteeName}`,
