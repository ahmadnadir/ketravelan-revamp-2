-- Allow active trip members (not only creator/admin) to update trip currency settings via RPC.
...
begin
  ...
  -- Permission model: creator OR active trip member can update trip currency settings.
  select (
    exists (
      select 1
      from public.trips t
